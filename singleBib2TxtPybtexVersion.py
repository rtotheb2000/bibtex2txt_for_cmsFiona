...
class Bib2Str:
    ...
    def strip_at_and_end(self):
        if self.bib_str == "":
            ...
            if self.bib_str[-4:] == "\"\n}\n":
                ...
                print(
                    "end of bib string could not be subtracted due to unrecognized format. conversion will not work.")
    ...
    def write_txt(self):
        if self.dictionary == "":
            print(
                "Nothing to write. Load a bib file using read_bib() before you use this module.")
        else:
            try:
                os.remove((self.dictionary["ID"]).lower() + ".txt")
            except:
                pass
            if not all(key in KEYS for key in self.dictionary.keys()):
                raise AttributeError(
                    "There is at least one required argument missing {}".format(KEYS))
            else:
                with io.open((self.dictionary["ID"]).lower() + ".txt", "w", encoding='utf8') as bib2txt_file:
                    ...
                    for i in ["ID", "title", "body", "author", "year", "month", "partOf", "source"]:
                        if i == "body":
                            ...
                            try:
                                bib2txt_file.write(self.dictionary[
                                                   "journal"] + " " + self.dictionary["volume"] + ", " + self.dictionary["pages"] + "\t")
                            except:
                                try:
                                    bib2txt_file.write(
                                        self.dictionary["booktitle"] + ", " + self.dictionary["pages"] + "\t")
                                except:
                                    print(
                                        "self.dictionary[", i, "] ", "returned error")
                        elif i == "source":
                            try:
                                bib2txt_file.write(
                                    "http://dx.doi.org/" + self.dictionary["doi"])
                            except:
                                try:
